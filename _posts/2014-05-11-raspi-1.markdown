...
**French summary:** _Cet article détaille l'installation logicielle
   ...
   [Raspberry Pi](http://www.raspberrypi.org). Comme il s'adresse
   essentiellement à un public d'informaticiens et d'électroniciens,
   presque tous anglophones, il est en Anglais._
...
![Electrical diagram]({{ site.url }}/img/elect.png)
...
needed to talk with other embedded systems: a 4-pins interface (GNV,
...
![Back view]({{ site.url }}/img/raspi_installed.jpg)
_Installation, viewed from the back_

![Front View]({{ site.url }}/img/victron_front.jpg)
_The same, viewed from the front_

![Wiring]({{ site.url }}/img/wiring-raspi-bmv.png)
_Wiring scheme_

Operating System
----------------

There are several OS distributions for Raspberry. I chose [Raspbian]()
because 1/ I've been using Debian derivatives for more than a decade
and 2/ [Mihini](http://www.eclipse.org/mihini) is ported and tested on
it.
...
[fully documented]() by Victron. Time to write a small program to
digests this.
...
to easily create and deploy M2M applications in [Lua](). Since I've
participated a lot in its architecture and implementation, the choice
was obvious, besides the many inherent qualities of that system--if I
may say so.

[Porting Mihini to a Raspberry]() isn't difficult, and is thoroughly
documented on Internet. As a bonus, and unsurprisingly for an Eclipse
product, it has an excellent support from the IDE, including
autocompletion, out-of-the box remote debugging, application
deployment etc.
...
But on second thoughs, the best code is the one you don't have to
...
![Reported on my phone]({{ site.url }}/img/screenshot-phone.png)
...
resulting program [in a gist]() anyway, but you shouldn't reuse
that. A much more proper version is available
[on github](https://github.com/fab13n/elorn_energy), with decent error
checking and a somewhat more polished API.
